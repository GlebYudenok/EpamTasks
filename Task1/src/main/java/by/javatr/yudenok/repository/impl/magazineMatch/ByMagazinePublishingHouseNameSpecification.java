...
public class ByMagazinePublishingHouseNameSpecification implements Specification<Magazine> {
    private String searchName;
    private static final Logger logger = LogManager.getLogger(ByMagazineNameWordInTitleSpecification.class);

    public ByMagazinePublishingHouseNameSpecification(String searchName) throws SpecificatorException {
        if(searchName == null){
    ...
    public boolean match(Magazine bean) {
        return bean.getPublishingHouseName().toLowerCase().equals(searchName.toLowerCase());
    }
}
